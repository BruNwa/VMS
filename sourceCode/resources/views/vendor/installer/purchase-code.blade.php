    ...
    <form method="post" action="{{ route('LaravelInstaller::purchase_code.check') }}" class="tabs-wrap">
        ...
        <div class="form-group {{ $errors->has('purchase_code') ? ' has-error ' : '' }}">
            <label for="purchase_code" class="ml-3">
                ...
                <a class="" data-toggle="modal" data-target="#myModal" title="Click"> <i class="fa fa-question-circle"></i> <span class="text-danger">({{ __(' Please click to see License activation process') }})</span></a>
            </label>
