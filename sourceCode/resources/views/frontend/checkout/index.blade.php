        ...
        <div class="pb-12 pt-12 md:pt-14 lg:pt-20">
            <div class="row">
                <div class="lg:col-6 md:col-6 col-12 md:pr-0 md:mt-16 mb-20">
                    <h1 class="text-2xl md:text-[32px] font-extrabold text-primary mb-6 leading-none">{{__('all.checkout-visitor')}}</h1>
                    <form action="{{ route('checkout.index') }}" method="POST">
                        @csrf
                        <label class="block text-black text-sm font-medium mb-2 required" for="email">{{__('all.visitor_id')}}</label>
