...
<section class="h-screen">
    <div class="container pb-8">
                        ...
                        <div class="flex justify-between items-center card-header">
                            <div class="flex items-center justify-center gap-[6px] company-details">
                                <div class="w-8 h-8 bg-white flex items-center logo-img rounded-lg">
                                    <img class="w-full logo" src="{{ themeSetting('site_logo') ? themeSetting('site_logo')->logo : asset('images/site_logo.png') }}" alt="logo">
                                </div>
                    ...
                    <div class="p-6 flex justify-start gap-x-[18px] card-body">
                        ...
                        <div class="user-details">
                            ...
                            <div class="mt-3 mb-3 phone">
                                <h2 class="text-[14px] font-semibold text-primary">{{ __('all.phone') }}</h2>
                                <p class="text-[14px] font-normal">+{{$visitingDetails->visitor->country_code}}{{$visitingDetails->visitor->phone}}</p>
                            </div>
                            <div>
                                <h2 class="text-[14px] font-semibold text-primary">{{ __('all.host') }}</h2>
        ...
        <div class="flex flex-wrap gap-y-2 sm:gap-y-0 justify-between items-center max-w-[571px] w-full mx-auto mt-7">
            <a href="{{ route('check-in') }}"><button type="reset" class="flex justify-start  bg-danger text-white px-6 py-3 rounded-3xl shadow-btnDanger text-lg font-bold leading-none">{{ __('all.back') }}</button></a>
            <div class="flex flex-wrap justify-end gap-4">
                @if($visitingDetails->visitor)
                <a href="#" id="print"><button class="bg-success text-white px-6 py-3 rounded-3xl shadow-btnSuccess text-lg font-bold leading-none">{{ __('all.print_id') }}</button></a>
                @endif
                <a href="{{ route('check-in') }}"><button type="submit" class="bg-primary text-lg font-bold text-white px-6 py-3 rounded-3xl shadow-btnNext leading-none">{{ __('all.home') }}</button></a>
            </div>
        </div>
    </div>
</section>
