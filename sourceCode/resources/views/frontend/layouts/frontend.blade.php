<!DOCTYPE>
<html>
@include('frontend.layouts.partials.head._head',['sitetitle' => $sitetitle])
<header class="mt-6">
    <div class="container">
        <div class="flex justify-between items-center py-2 px-2 backdrop-blur-xl bg-[rgba(255,255,255,0.6)] rounded-[32px]">
            <a href="{{route('/')}}" class="w-28">
                <img src="{{ themeSetting('site_logo') ? themeSetting('site_logo')->logo : asset('images/site_logo.png') }}" alt="logo" class="w-full">
            </a>
            <div class="lg:flex items-center gap-x-12 hidden">
                <nav class="lg:flex items-center gap-x-12 font-semibold text-lg">
                    <a href="{{route('check-in.pre.registered')}}" class="hover:text-primary">{{__('all.have_appoinment')}}</a>
                    <a href="{{route('check-in.return')}}" class="hover:text-primary ">{{__('all.been_here_before')}}</a>
                    @if(auth()->user())
                    <a href="{{route('checkout.index')}}" class="hover:text-primary ">{{__('all.check_out')}}</a>
                    @endif
                </nav>
                <div class="dropdown">
                ...
                @if(auth()->user())
                <a href="{{route('admin.dashboard.index')}}"><button class="bg-primary text-white rounded-[23.5px] px-6 py-3 leading-tight text-lg font-semibold">{{__('all.go_to_dashboard')}}</button></a>

                @else
                <a href="{{route('login')}}"><button class="bg-primary text-white rounded-[23.5px] px-6 py-3 leading-tight text-lg font-semibold">{{__('all.login')}}</button></a>
