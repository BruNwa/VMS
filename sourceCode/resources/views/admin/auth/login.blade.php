                ...
                <div class="col-12 md:col-6 !py-0">
                    <div class="py-6 px-4 md:px-6 md:pr-0">
                        <form method="POST" action="{{ route('login') }}">
                            @csrf
                            <h1 class="text-2xl sm:text-[32px] font-extrabold text-primary mb-8 md:mb-12 leading-none">{{ __('all.login') }}</h1>
                            <div class="mb-6">
                                <label class="block tracking-wide text-black text-sm font-medium mb-2 required" for="email">{{ __('all.email') }}</label>
                            ...
                            <div class="mb-6">
                                <label class="block tracking-wide text-black text-sm font-medium mb-2 required" for="password">{{ __('all.password') }}</label>
                            ...
                            <div class="flex justify-between mb-7">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <label for="checkbox-1" class="font-medium text-sm"> {{ __('all.remember_me') }}</label>
                                </div>
                                <span class="font-medium text-sm text-primary"><a href="{{route('password.request')}}" class="custom-control-labe">{{ __('all.reset_password') }}?</a></span>
                            </div>
                            <button type="submit" class="text-lg font-bold leading-snug text-white bg-primary rounded-[23.5px] px-6 py-3 w-full shadow-btnNext">{{ __('all.login') }}</button>
                        </form>
                        @if(env('DEMO'))
                        <p class="text-base font-bold mt-12 mb-4">{{ __('all.for_quick_demo_login_click_below') }}</p>
                        <div class="flex flex-shrink gap-x-2 sm:gap-x-4 justify-between sm:justify-start">
                            <button id="demo-admin" class="bg-[#4F97EC] text-white rounded-3xl px-4 py-3 shadow-btnAdmin text-sm md:text-base font-bold">{{ __('all.admin') }}</button>
                            <button id="demo-employee" class="bg-[#EC874F] text-white rounded-3xl px-4 py-3 shadow-btnEmployee text-sm md:text-base font-bold">{{ __('all.employee') }}</button>
                            <button id="demo-reception" class="bg-[#944FEC] text-white rounded-3xl px-4 py-3 shadow-btnReception text-sm md:text-base font-bold">{{ __('all.reception') }}</button>
                        </div>
                        @endif
