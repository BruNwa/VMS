...
<div class="col-12">
    <div class="db-card">
        <div class="db-card-header">
            <h3 class="db-card-title">{{ __('all.pre_registers') }}</h3>
        </div>
            ...
            <form action="{{ route('admin.pre-registers.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="form-col-12 sm:form-col-6 md:form-col-4">
                        ...
                        <select id="employee_id" name="employee_id" class="db-field-control select2 @error('employee_id') invalid @enderror">
                            ...
                            <option value="{{ $employee->id }}" {{ old('employee_id') == $employee->id ? 'selected' : '' }}>{{ $employee->name }}
                                ({{ optional($employee->department)->name }})</option>
                            @endforeach
                        </select>
                        @error('employee_id')
                        <small class="db-field-alert">{{ $message }}</small>
                    ...
                    <div class="form-col-12 sm:form-col-6 md:form-col-4">
                        ...
                        <input type="date" name="expected_date" id="expected_date" class="db-field-control @error('expected_date') invalid @enderror" value="{{ old('expected_date') }}">
                        @error('expected_date')
                        <small class="db-field-alert">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="form-col-12 sm:form-col-6 md:form-col-4">
                        ...
                        <input type="time" name="expected_time" class="db-field-control @error('expected_time') invalid @enderror" id="expected_time" value="{{ old('expected_time') }}">
                        @error('expected_time')
                        <small class="db-field-alert">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="form-col-12 sm:form-col-6 md:form-col-4">
                        ...
                        <input type="text" name="comment" class="db-field-control @error('comment') invalid @enderror" value="{{ old('comment') }}">
                        @error('comment')
                        <small class="db-field-alert">{{ $message }}</small>
                        @enderror
                    </div>
